...
from .viewer_session import DebugViewerSession
...
def canonicalize_contact_frame_wrench(
    ...
    *,
    ...
    """Canonicalize a MuJoCo contact while preserving the object world wrench.

    MuJoCo stores contact axes as rows and returns the local wrench acting on
    ``geom2``. The public representation uses a right-handed frame whose first
    axis points from hand to object and a local wrench acting on the object.

    Args:
        raw_frame: MuJoCo frame as a flat 9-vector or row-axis 3x3 array.
        raw_wrench: MuJoCo local force/torque vector acting on ``geom2``.
        hand_is_geom1: Whether the hand is ``geom1`` in this contact.
        tolerance: Tolerance for frame and normal-force invariants.

    Returns:
        Pair ``(world_frame, local_object_wrench)`` with frame axes as columns.

    Raises:
        ValueError: If a frame invariant fails or normal compression is negative.
    """
    raw_world_frame = np.asarray(raw_frame, dtype=float).reshape(3, 3).T.copy()
    wrench = np.asarray(raw_wrench, dtype=float).reshape(6).copy()
    identity = np.eye(3)
    if not np.allclose(raw_world_frame.T @ raw_world_frame, identity, atol=tolerance, rtol=0):
        raise ValueError("MuJoCo contact frame is not orthonormal.")
    if not np.isclose(np.linalg.det(raw_world_frame), 1.0, atol=tolerance, rtol=0):
        raise ValueError("MuJoCo contact frame is not right-handed.")

    if hand_is_geom1:
        canonical_frame = raw_world_frame
        canonical_wrench = wrench
    else:
        # Flip the normal and one tangent to retain a proper rotation. Apply
        # action-reaction (-) and the same basis transform (S) to both force
        # and torque, so R_canonical @ wrench_canonical stays the object wrench.
        canonical_frame = raw_world_frame @ _CONTACT_ORDER_SWAP
        canonical_wrench = np.concatenate(
            [
                (-_CONTACT_ORDER_SWAP) @ wrench[:3],
                (-_CONTACT_ORDER_SWAP) @ wrench[3:],
            ]
        )

    if not np.allclose(canonical_frame.T @ canonical_frame, identity, atol=tolerance, rtol=0):
        raise ValueError("Canonical contact frame is not orthonormal.")
    if not np.isclose(np.linalg.det(canonical_frame), 1.0, atol=tolerance, rtol=0):
        raise ValueError("Canonical contact frame is not right-handed.")
    if canonical_wrench[0] < -tolerance:
        raise ValueError(f"Canonical object contact has negative normal compression force {canonical_wrench[0]}.")
    return canonical_frame, canonical_wrench
...
class MjHO:
    ...
    def __init__(
        ...
        self.debug_viewer = debug_viewer_session
        self._owns_debug_viewer_session = False
        self.debug_render = None
        if debug_viewer:
            # Direct MjHO callers keep the historical native-viewer default;
            # task runners pass a shared session to enable persistent mjviser.
            if self.debug_viewer is None:
                self.debug_viewer = DebugViewerSession(backend="mujoco")
                self._owns_debug_viewer_session = True
            self.debug_viewer.attach(self.model, self.data)
    ...
    def get_contact_info(self, hand_qpos, obj_pose, obj_margin=0):
        ...
        for contact in self.data.contact:
            ...
            if (body1_id > world_id and body1_id <= hand_id and body2_id == object_id) or (
                ...
                # Canonicalize even this force-free query so initialization and
                # runtime paths cannot drift in their geom-order convention.
                hand_is_geom1 = body2_id == object_id
                contact_frame, _ = canonicalize_contact_frame_wrench(
                    contact.frame,
                    np.zeros(6),
                ...
                ho_contact.append(
                    {
                        ...
                        "contact_normal": contact_frame[:, 0],
    ...
    def get_curr_contact_info(self):
        ...
        for contact_id, contact in enumerate(self.data.contact):
            ...
            if (body1_id > world_id and body1_id <= hand_id and body2_id == object_id) or (
                ...
                # Keep the public identities hand-first/object-second, while the
                # pure transform handles MuJoCo's actual geom ordering.
                hand_is_geom1 = body2_id == object_id
                if hand_is_geom1:
                    hand_body_name = body1_name.removeprefix(self.hand_prefix)
                    ...
                    obj_body_name = body1_name
                contact_frame, contact_force = canonicalize_contact_frame_wrench(
                    contact.frame,
                    contact_force,
                    hand_is_geom1=hand_is_geom1,
                )
                ...
                # Calculate the canonical contact frame in the hand body frame.
    ...
    def close_view_and_render(self):
        if self.debug_viewer is not None:
            self.debug_viewer.detach()
            if self._owns_debug_viewer_session:
